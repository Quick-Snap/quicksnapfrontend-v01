...
uniform float uFit; // 0 = cover (crop), 1 = contain (letterbox)
...
// object-fit: contain — full image visible, letterbox/pillarbox as needed
...
vec2 fitUV(vec2 uv, vec2 res, vec2 img) {
  return uFit > 0.5 ? containUV(uv, res, img) : coverUV(uv, res, img);
}

vec3 sampleFit(sampler2D tex, vec2 uv, float ca) {
...
void main() {
  ...
  // Subtle drift/zoom only in cover mode — contain must not crop edges.
  if (uFit < 0.5) {
  ...
  vec3 colC = sampleFit(tCurrent, sC, ca);
  vec3 colN = sampleFit(tNext, sN, ca);

  vec3 col = mix(colC, colN, m);

  float vig = smoothstep(1.25, 0.25, length(uv - 0.5));
  float vigAmt = uFit > 0.5 ? 0.0 : 0.28;
  col = mix(col, uOverlay, (1.0 - vig) * vigAmt);
...
class MorphEngine {
  constructor(container, { items, startIndex, reducedMotion, getOptions, onIndexChange, onAnnounce, dprCap }) {
    ...
    this.items.forEach((item, index) => {
      ...
      const apply = (img) => {
        try {
          ...
          /* tainted / GL error — keep fallback texture */
        }
      };

      // Cross-origin CDNs (CloudFront) often omit CORS headers. Loading via our
      // same-origin `/api/image-proxy` keeps bitmaps readable by WebGL.
      let src = item.image;
      try {
        if (typeof window !== 'undefined') {
          const absolute = new URL(item.image, window.location.href);
          if (absolute.origin !== window.location.origin) {
            src = `/api/image-proxy?url=${encodeURIComponent(absolute.href)}`;
          }
        }
      } catch {
        /* keep original src */
  ...
  loop(t) {
    try {
      ...
      // Avoid crashing the rAF loop on a single tainted upload.
    }
    this.raf = requestAnimationFrame(this.boundLoop);
  }
  ...
  goTo(dir) {
    // Button/keyboard navigation must win over an in-progress click-drag.
    this.cancelDrag();
    ...
    this.animating = true;
    // Update captions/dots only — do NOT notify React parents mid-tween
    // (parent setState can remount/destroy the WebGL engine and freeze nav).
    this.announce(target, false);
  ...
  announce(index, notifyParent = true) {
    if (index !== this.shownIndex) {
      this.shownIndex = index;
      this.onAnnounce?.(index);
    }
    // Mid-tween / drag updates keep React parents out (avoids remounting WebGL).
    if (notifyParent) this.onIndexChange?.(index);
  }
  ...
  drag(ndx) {
    ...
    // Keep React parents out of the drag loop — only update local captions/dots.
    this.announce(progress > 0.5 ? this.wrap(this.current + dir) : this.current, false);
  }
  ...
  endDrag() {
    ...
    if (p > 0.4) {
      ...
      this.announce(this.current, false);
      this.tween = gsap.to(this.program.uniforms.uProgress, {
        value: 0,
        duration,
        ease: 'power2.out',
...
export default function MorphSlider({
  ...
  // Recreate only when slide URLs change — not when parent syncs the selected index.
  const itemsKey = Array.isArray(items) ? items.map(item => item?.image ?? '').join('\0') : '';
  ...
  useEffect(() => {
    ...
    const slideItems = itemsRef.current;
    const safeStart = Math.max(0, Math.min(startIndexRef.current, Math.max(slideItems.length - 1, 0)));
    ...
    const engine = new MorphEngine(containerRef.current, {
      ...
      onAnnounce: nextIndex => setIndex(nextIndex),
      onIndexChange: nextIndex => onIndexChangeRef.current?.(nextIndex)
    });
  ...
  // Keep internal caption/dot index in sync without tearing down the engine.
  ...
  useEffect(() => {
    ...
    const onDown = e => {
      // Ignore non-primary buttons and clicks that originate on controls.
      if (e.button != null && e.button !== 0) return;
      if (e.target?.closest?.('.morph-slider-btn, .morph-slider-dot')) return;
